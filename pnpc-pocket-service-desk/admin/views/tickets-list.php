...
if (!$is_trash_view && !empty($tickets_paginated)) {
	foreach ($tickets_paginated as $ticket) {
...
// Helper function to render a ticket row
if (!function_exists('pnpc_psd_render_ticket_row_view')) {
	function pnpc_psd_render_ticket_row_view($ticket, $is_trash_view, $badge_counts, $is_closed = false) {
		...
		if (isset($badge_counts[$ticket->id])) {
			...
			if (! $is_trash_view && current_user_can('pnpc_psd_view_tickets')) {
				...
				if (empty($last_view_meta)) {
					...
					$last_view_time = is_numeric($last_view_meta) ? intval($last_view_meta) : strtotime($last_view_meta);
		...
		$row_classes = $is_closed ? ' class="pnpc-psd-ticket-closed"' : '';
		?>
...
<div class="wrap">
	...
	<table class="wp-list-table widefat fixed striped" id="pnpc-psd-tickets-table">
		...
		<tbody>
			<?php if ($is_trash_view) : ?>
				<?php // Trash view - no separation ?>
				<?php if (! empty($tickets_paginated)) : ?>
					<?php foreach ($tickets_paginated as $ticket) : ?>
						<?php pnpc_psd_render_ticket_row_view($ticket, $is_trash_view, $badge_counts); ?>
					<?php endforeach; ?>
				<?php else : ?>
					<tr>
						<td colspan="<?php echo current_user_can('pnpc_psd_delete_tickets') ? '6' : '5'; ?>">
							<?php esc_html_e('No tickets in trash.', 'pnpc-pocket-service-desk'); ?>
						</td>
					</tr>
				<?php endif; ?>
			<?php else : ?>
				<?php // Non-trash view - with active/closed separation ?>
				...
				<?php else : ?>
					<?php // Render active tickets first ?>
					<?php if ($has_active) : ?>
						<?php foreach ($active_tickets as $ticket) : ?>
							<?php pnpc_psd_render_ticket_row_view($ticket, $is_trash_view, $badge_counts, false); ?>
						<?php endforeach; ?>
					<?php endif; ?>
					
					<?php // Render divider if we have both active and closed ?>
					<?php if ($has_active && $has_closed) : ?>
					...
					<?php // Render closed tickets ?>
					<?php if ($has_closed) : ?>
						<?php foreach ($closed_tickets as $ticket) : ?>
							<?php pnpc_psd_render_ticket_row_view($ticket, $is_trash_view, $badge_counts, true); ?>
						<?php endforeach; ?>
					<?php endif; ?>
				<?php endif; ?>
			<?php endif; ?>
