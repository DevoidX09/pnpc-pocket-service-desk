	...
	public function display_setup_wizard_page() {
		...
		if ( 'POST' === sanitize_text_field( wp_unslash( $_SERVER['REQUEST_METHOD'] ?? '' ) ) && isset( $_POST['pnpc_psd_setup_nonce'] ) ) {
			...
			if ( 'create' === $mode ) {
				...
				$page_id = isset( $_POST['existing_page_id'] ) ? absint( wp_unslash( $_POST['existing_page_id'] ) ) : 0;
				if ( $page_id > 0 ) {
					update_option( 'pnpc_psd_dashboard_page_id', (int) $page_id, false );
					update_option( 'pnpc_psd_setup_completed_at', time(), false );
					wp_safe_redirect( admin_url( 'admin.php?page=pnpc-service-desk-setup&step=done' ) );
		...
		// Fallback candidate by common slug.
		$dashboard_slug_candidate = get_page_by_path( 'dashboard' );
...
public function display_tickets_page()
	...
	*/
	public function display_ticket_detail_page()
	{
		...
		// Previous ticket: highest ID less than current (most recent before this one).
		$prev_ticket_id = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT id FROM {$tickets_table} WHERE id < %d AND deleted_at IS NULL ORDER BY id DESC LIMIT 1",
				$ticket_id
			)
		);
		$prev_ticket_id = $prev_ticket_id ? absint( $prev_ticket_id ) : 0;

		// Next ticket: lowest ID greater than current (next one after this).
		$next_ticket_id = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT id FROM {$tickets_table} WHERE id > %d AND deleted_at IS NULL ORDER BY id ASC LIMIT 1",
				$ticket_id
			)
		);
