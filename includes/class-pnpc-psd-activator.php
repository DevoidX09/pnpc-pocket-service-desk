...
class PNPC_PSD_Activator {
	...
	public static function ensure_delete_reason_columns() {
		...
		foreach ( $needs as $col => $sql ) {
			...
			if ( empty( $column_exists ) ) {
				// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.SchemaChange, WordPress.DB.PreparedSQL.NotPrepared -- Plugin-controlled schema SQL fragment.
				$wpdb->query( $sql );
			}
		}
	...
	private static function upgrade_to_1_5_0() {
		...
		foreach ( $columns as $col => $sql ) {
			...
			if ( ! $exists ) {
				// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.SchemaChange, WordPress.DB.PreparedSQL.NotPrepared -- Plugin-controlled schema SQL fragment required for activation/update schema changes.
				$wpdb->query( $sql );
			}
		}
